...
- **Hex siempre en minúsculas:** todos los valores de color (`#rrggbb` o `#rrggbbaa`) en `src/theme-config.js` se escriben en minúsculas (tooling del author). Al final de cada cambio al source of truth, normaliza cualquier hex con letras en mayúscula. VS Code muestra los colores formateados en el inspector independientemente del caso del archivo, así que el estándar interno no afecta la salida visual.
